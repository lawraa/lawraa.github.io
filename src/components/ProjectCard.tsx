...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <>
      <article
        className="rounded-xl overflow-hidden card"
      ...
      >
        <div className="p-5">
          <div className="flex flex-col sm:flex-row sm:items-center gap-5">
            ...
            {project.imageUrl && (
              <div
                className={`shrink-0 w-full h-48 sm:w-44 sm:h-44 rounded-lg overflow-hidden border-4${project.imageFit === 'contain' ? ' p-2' : ''}`}
                style={{
              ...
              >
                <img
                  ...
                  alt={project.title}
                  className={`w-full h-full cursor-zoom-in ${project.imageFit === 'contain' ? 'object-contain' : 'object-cover'}`}
                  onClick={() => setLightbox(project.imageUrl)}
                />
              </div>
            )}
      ...
      {/* Lightbox — portalled to body to escape any parent transform/stacking context */}
